...
func (o *UnblockConfigURL) Build() (*url.URL, error) {
	...
	var _path = "/config/{config_id}/unblock"

	configID := o.ConfigID
	if configID != "" {
		_path = strings.Replace(_path, "{config_id}", configID, -1)
	} else {
		return nil, errors.New("configId is required on UnblockConfigURL")
	}
...
func (o *UnblockConfigURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UnblockConfigURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UnblockConfigURL")
	}
