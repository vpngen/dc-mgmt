...
func (o *BlockConfigURL) Build() (*url.URL, error) {
	...
	var _path = "/config/{config_id}/block"

	configID := o.ConfigID
	if configID != "" {
		_path = strings.Replace(_path, "{config_id}", configID, -1)
	} else {
		return nil, errors.New("configId is required on BlockConfigURL")
	}
...
func (o *BlockConfigURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BlockConfigURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BlockConfigURL")
	}
