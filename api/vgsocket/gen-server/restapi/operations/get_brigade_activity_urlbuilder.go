...
func (o *GetBrigadeActivityURL) Build() (*url.URL, error) {
	...
	var _path = "/brigade/{brigade_id}/activity"

	brigadeID := o.BrigadeID
	if brigadeID != "" {
		_path = strings.Replace(_path, "{brigade_id}", brigadeID, -1)
	} else {
		return nil, errors.New("brigadeId is required on GetBrigadeActivityURL")
	}
...
func (o *GetBrigadeActivityURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetBrigadeActivityURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetBrigadeActivityURL")
	}
